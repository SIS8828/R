...
html_cont <- as.data.frame(html_cont) # dataframe형변환 
str(html_cont)
head(html_cont)
...
getwd() #"C:/workspaces/R/output"
# setwd("C:/workspaces/R/output")

library(RSADBE)
data("Severity_Counts") # Severity_Counts 데이터셋 가져오기 
Severity_Counts
...
# 단계3 : 'row.names=F' 속성을 이용하여 행이름 제거하여 저장한다.
write.table(studenttx, "stdt2.txt", row.names = F)

# 단계4 : 'quote=F' 속성을 이용하여 따옴표를 제거하여 저장한다.
...
write.csv(st.df, "stdf.csv", row.names = F, quote = F) # 행이름/따옴표 제거 
