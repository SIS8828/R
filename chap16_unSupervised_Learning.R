...
##################################################
## Chapter15_1. 군집 분석(Clustering Analysis)
##################################################

## 1. 유클리드안 거리
# 유클리드안 거리(Euclidean distance)는 두 점 사이의 거리를 계산하는 방법으로, 이 거리를 이용하여 유클리드 공간을 정의한다.
...
# 단계 1 : 데이터 셋 가져오기
interview <- read.csv("C:/workspaces/Rwork/data/interview.csv", header=TRUE)
names(interview)
...
# 단계 2 : 그룹 요약통계량  
table(interview$no)
summary(g1) # 불합격:5, 자격증 없음+있음.
summary(g2) # 합격:5, 자격증 있음.
summary(g3) # 불합격:5, 자격증 없음.
...
# 단계 3 :  비계층적 군집분석
result2 <- kmeans(mydia, 3) # 3개 군집수 적용.
...
#################################################
## Chapter15_2. 연관분석(Association Analysis)
#################################################
...
btran <- read.transactions("C:/workspaces/Rwork/data/demo_basket",format="basket",sep=",") # 트랜잭션 구분자(Transaction ID) 없이 상품으로만 구성된 데이터 셋을 대상으로 트랜잭션 객체를 생성할 경우 format="basket" 속성 지정.
