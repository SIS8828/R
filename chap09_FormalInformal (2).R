...
# inspect(myCorpusfacebook_txt)
# Sys.setlocale('LC_ALL','C')
"""

# 단어길이 2개 이상인 단어만 선별하여 matrix 자료구조로 변경.
myCorpusfacebook_txt <- TermDocumentMatrix(myCorpusfacebook, control=list(wordLengths=c(2,Inf)))

myCorpusfacebook_txt

# Encoding(myCorpusfacebook_txt$dimnames$Terms) = 'UTF-8'
# as.matrix(myCorpusfacebook_txt)
...
# myCorpusfacebook_txt <- tm_map(myCorpusfacebook, PlainTextDocument)
myCorpusfacebook_txt <- myCorpusfacebook

myCorpusfacebook_txt <- TermDocumentMatrix(myCorpusfacebook_txt,control = list(wordLengths=c(2, Inf)))
...
# 2.2 연관어 분석 
# - 연관어 분석:연관규칙을 적용하여 특정 단어와 연관성이 있는 단어들을 선별하여 네트워크 형태로 시각화하는 과정.
...
lword <- sapply(lword, filter2) # 단어 길이가 1이하 또는 5이상인 단어는 제거 
...
# - 연관 분석을 위해서는 추출된 단어를 대상으로 트랜잭션 형식으로 자료구조 변환.
...
# 단어간 연관규칙 산출
tranrules <- apriori(wordtable,parameter = list(support=0.5,conf=0.8))
...
apr1<- apriori(Adult,parameter = list(support=0.1,target="frequent"),appearance = list(none=c("income=small","income=large"),default="both")) # set of 2066 itemsets 
...
# 연관어 시각화를 위한 igraph 설치
install.packages("igraph")

library(igraph)

#graph.edgelist 보기 - 연관 단어를 정점 형태의 목록 제공
ruleg <- graph.edgelist(rulemat[c(5:59),],directed = F)
ruleg
